classdef MinTimeCoGDoubleTrack < handle
    ...
    methods (Access = public)
        function obj = MinTimeCoGDoubleTrack(parameters)
            ...
            obj.F_x01_min = parameters.F_x01_min;
            obj.F_x02_min = parameters.F_x02_min;
            obj.F_x03_min = parameters.F_x03_min;
            obj.F_x04_min = parameters.F_x04_min;
            obj.N = parameters.N;
            obj.p = parameters.p;
            ...
            obj.U_guess = parameters.U_guess;
            obj.X_guess = parameters.X_guess;
            obj.tf_guess = parameters.tf_guess;
        ...
        function x_dot = vehicle_model(obj, x, u)
            ...
            x_dot = [Xp_dot; Yp_dot; phi_dot; r_dot; vx_dot; vy_dot; del_dot];
        end

        function optimizer = OPTFormulation(obj)

            opti = casadi.Opti(); 
            U  = opti.variable(obj.D_control, obj.N); 
            X  = opti.variable(obj.D_state, obj.N + 1);
            tf = opti.variable( ); 
            ...
            X0 = opti.parameter(obj.D_state - 1, 1); 
            ...
            opti.subject_to(obj.F_x01_min <= F_x1 <= 0);
            opti.subject_to(obj.F_x02_min <= F_x2 <= 0);
            opti.subject_to(obj.F_x03_min <= F_x3 <= obj.F_x03_max);
            opti.subject_to(obj.F_x04_min <= F_x4 <= obj.F_x04_max);
            opti.subject_to(obj.delta_min <= delta(2:end) <= obj.delta_max); 
            opti.subject_to(obj.delta_dot_min <= delta_dot <= obj.delta_dot_max); 
            ...
            opti.subject_to(0 <= Yp);
            opti.subject_to(0 <= vx);

            opti.set_initial(tf, obj.tf_guess);
            opti.set_initial(X,  obj.X_guess);
            opti.set_initial(U,  obj.U_guess);
